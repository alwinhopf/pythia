...
x_coords = np.arange(np.floor(minx), np.ceil(maxx), 0.25)
y_coords = np.arange(np.floor(miny), np.ceil(maxy), 0.25)
...
output_shapefile = os.path.join(input_dir, "USA_contiguous_states_gridpoints_0.25degree.shp")
...
plt.title("0.5 * 0.5 Degree Grid Points Over Shapefile")
plt.xlabel("Longitude")
plt.ylabel("Latitude")

# Save the plot as a JPEG file
output_figure = os.path.join(input_dir, "USA_contiguous_states_gridpoints_0.25degree.jpg")
