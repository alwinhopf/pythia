...
x_coords = np.arange(np.floor(minx), np.ceil(maxx), 0.1)
y_coords = np.arange(np.floor(miny), np.ceil(maxy), 0.1)
...
output_shapefile = os.path.join(input_dir, "USA_contiguous_states_gridpoints_0.1degree.shp")
...
output_figure = os.path.join(input_dir, "USA_contiguous_states_gridpoints_0.1degree.jpg")
